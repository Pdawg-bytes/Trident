...
namespace Trident.CodeGeneration.Decoding
{
    ...
    {
        internal static ThumbGroup DetermineThumbGroup(ushort instruction) =>
            instruction switch
            {
                ...
                ushort i when (i & 0xF800) == 0x4800 => ThumbGroup.LoadStorePCRelative,
                ushort i when (i & 0xF200) == 0x5000 => ThumbGroup.LoadStoreImmOffset,
                ushort i when (i & 0xF200) == 0x5200 => ThumbGroup.LoadStoreSigned,
                ushort i when (i & 0xE000) == 0x6000 => ThumbGroup.LoadStoreImmOffset,
                ushort i when (i & 0xF000) == 0x8000 => ThumbGroup.LoadStoreImmOffset,
