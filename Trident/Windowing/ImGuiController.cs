...
public class ImGuiController : IDisposable
{
        ...
        ImGuiStyleConfig style)
    {
        ...
        _compatProfile = (GL.GetInteger((GetPName)All.ContextProfileMask) & (int)All.ContextCompatibilityProfileBit) != 0;
    ...
    private void ApplyStyle(ImGuiStyleConfig config)
    {
        ...
        style.ItemSpacing = new System.Numerics.Vector2(config.ItemSpacing[0], config.ItemSpacing[1]);
        style.CellPadding = new System.Numerics.Vector2(config.CellPadding[0], config.CellPadding[1]);
        style.FramePadding = new System.Numerics.Vector2(config.FramePadding[0], config.FramePadding[1]);
        style.WindowTitleAlign = new System.Numerics.Vector2(config.WindowTitleAlign[0], config.WindowTitleAlign[1]);

        foreach (var kvp in config.Colors)
        {
            var col = (ImGuiCol)Enum.Parse(typeof(ImGuiCol), kvp.Key);
            var vec = kvp.Value;
            style.Colors[(int)col] = new System.Numerics.Vector4(vec[0], vec[1], vec[2], vec[3]);
        }
    }
...
void main()
{
    ...
    /// <summary>
    /// Alerts ImGui of any input to the OpenTK window.
    /// </summary>
    /// <param name="wnd">The current Window receiving input.</param>
    ...
    /// <summary>
    /// Alerts ImGui of a KeyUp/KeyDown event.
    /// </summary>
    /// <param name="key">The OpenTK key.</param>
    /// <param name="isDown">If the key is pressed.</param>
    public void KeyEvent(Keys key, bool isDown)
    {
        ImGuiIOPtr io = ImGui.GetIO();
    ...
    private void RenderImDrawData(ImDrawDataPtr draw_data)
    {
        ...
        if (_glVersion <= 310 || _compatProfile)
        {
    ...
    {
        ImGuiKey ConvertToImGuiRange(Keys keyToConvert, Keys startKey1, ImGuiKey startKey2)
        {
        ...
        return key switch
        {
            >= Keys.F1 and <= Keys.F24 => ConvertToImGuiRange(key, Keys.F1, ImGuiKey.F1),
            >= Keys.KeyPad0 and <= Keys.KeyPad9 => ConvertToImGuiRange(key, Keys.KeyPad0, ImGuiKey.Keypad0),
            >= Keys.A and <= Keys.Z => ConvertToImGuiRange(key, Keys.A, ImGuiKey.A),
            >= Keys.D0 and <= Keys.D9 => ConvertToImGuiRange(key, Keys.D0, ImGuiKey._0),
